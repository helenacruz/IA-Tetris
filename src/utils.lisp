...
;;pecas novas
;;peca bomba
(defconstant peca-bomba (make-array (list 1 1) :initial-contents '((T))))
;;peca u
(defconstant peca-u0 (make-array (list 3 3) :initial-contents '((T T T)(T nil T)(T nil T))))
(defconstant peca-u1 (make-array (list 3 3) :initial-contents '((T T T)(T nil nil)(T T T))))
(defconstant peca-u2 (make-array (list 3 3) :initial-contents '((T nil T)(T nil T)(T T T))))
(defconstant peca-u3 (make-array (list 3 3) :initial-contents '((T T T)(nil nil T)(T T T))))
	...
	(let ((estado estado-inicial))
		(do () ((or (estado-final-p estado) (null lista-accoes)))
			(when desenha
				...
				(read-char))
    ;  (print (estado-pontos estado))
			(setf estado (resultado estado (first lista-accoes)))
			(print estado)
			(print (first lista-accoes))
...
(defvar e1 (make-estado :tabuleiro t1 :pecas-por-colocar '(i o j u bomba i)))

(defvar p1
	(make-problema :estado-inicial (make-estado :tabuleiro t1 :pecas-por-colocar '(i o j u bomba i))
				   :solucao #'solucao
				   :accoes #'accoes
				   :resultado #'resultado
